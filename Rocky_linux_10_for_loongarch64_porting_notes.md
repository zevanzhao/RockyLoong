...
现在已经有了很多其他龙架构下的操作系统，如Debian、AOSC以及Fedora Remix。我准备采用本地编译、而非交叉编译的方式构建临时操作系统。我准备采用Debian
作为host系统，开始我的构建。

首先，搭建系统环境。仿照孙老师的书，在Debian
系统下，需要安装一些必要的软件
...
创建专用用户，并进行设置。在孙老师的书里，为了构建交叉工具链，专门使用dnf命令安装了一个小型的初始系统，使用chroot来运行。我准备跳过这个步骤，直接使用原生的系统，但新建一个用户来做这个事情。
```bash
 groupadd rocky                                                                     
...
关于CROSS_HOST, CROSS_TARGET, CROSS_BUILD三个概念：

CROSS_BUILD 即当前运行的平台。

CROSS_HOST即交叉编译链运行的平台。CROSS_HOST所设置的前缀，将会用于交叉工具链。

CROSS_TARGET即工具链所生成的二进制程序所要运行的平台。
...
## libxcrypt {#libxcrypt .unnumbered}

tcsh和pam都需要libcrypt, 用libxcrypt中的替代。完成。

注意，\--libdir需要设置为/tools/lib64
...
安装以后，pam_appl.h文件不在security目录中。需要调整includedir。
...
## 注意：安装了pam后，需要在/tools/etc/pam.d中，进行一系列的设置，否则系统将无法启动。这一步可以在配置临时系统的时候操作。 {#注意安装了pam后需要在toolsetcpam.d中进行一系列的设置否则系统将无法启动这一步可以在配置临时系统的时候操作 .unnumbered}

## openssh {#openssh .unnumbered}

缺少security/pam_appl.h文件。需要安装额外的程序，pam

加上了\--with-pam参数，完成

检查：ls /tools/bin/ssh\* /tools/sbin/ssh\*

/tools/bin/ssh /tools/bin/ssh-agent /tools/bin/ssh-keyscan

/tools/bin/ssh-add /tools/bin/ssh-keygen /tools/sbin/sshd

## sudo  {#sudo .unnumbered}
...
## e2fsprogs  {#e2fsprogs .unnumbered}
...
## inih  {#inih .unnumbered}

按照https://github.com/sunhaiyong1978/CLFS-for-LoongArch/blob/main/CLFS_For_LoongArch64.md

中的脚本，编译安装

检查：ls /tools/lib64/libinih.so\*

/tools/lib64/libinih.so /tools/lib64/libinih.so.0

## uuid {#uuid .unnumbered}

需要更新configure.guess以及 configure.sub文件

检查：ls /tools/lib64/libossp-uuid.\*
...
## userspace-rcu {#userspace-rcu .unnumbered}
...
## xfsprogs {#xfsprogs .unnumbered}

使用了ini.h，需要安装libinih开发包。

使用了uuid.h,需要安装uuid。

需要urcu.h安装userspace-rcu

注意，xfsprogs使用的uuid,由util-linux提供，不是ossp-uuid。

完成

检查：ls /tools/sbin/\*xfs

/tools/sbin/fsck.xfs /tools/sbin/mkfs.xfs

## dosfstools {#dosfstools .unnumbered}

完成

检查：ls /tools/sbin/\*vfat

/tools/sbin/fsck.vfat /tools/sbin/mkfs.vfat

## bison {#bison .unnumbered}

完成

检查：ls /tools/bin/{bison,yacc}

/tools/bin/bison /tools/bin/yacc

## check {#check .unnumbered}

configure的时候，添加\--disable-build-docs选项

ls /tools/bin/checkmk

/tools/bin/checkmk

## diffutils {#diffutils .unnumbered}

完成

检查：ls /tools/bin/diff\*

/tools/bin/diff /tools/bin/diff3

## make {#make .unnumbered}
...
## patch {#patch .unnumbered}

需要更新config.sub和config.guess。完成
...
## audit {#audit .unnumbered}

需要根据spec文件，编写编译脚本，去掉一些功能。
...
## nettle {#nettle .unnumbered}

需要autoreconf -ifv,并且在configure时，去掉 sm3和sm4支持。
...
rpm
4.15采用configure来生成Makefile，但4.19改用了cmake。因此，交叉编译的方式需要进行改变。

[创建loongarch64-toolchain.cmake。]{.mark}
...
## 配置rpm {#配置rpm .unnumbered}

完成

## systemd {#systemd .unnumbered}

按照CLFS以及豹书进行操作。需要设置PKG_CONFIG_PATH

需要将version_no_tilde设置为257

rpmbuild -D \"version_no_tilde 257\" -bp \~/rpmbuild/SPECS/systemd.spec
\--nodeps
...
## shadow-utils {#shadow-utils .unnumbered}
...
## grub {#grub-1 .unnumbered}

[完成]{.mark}

[ls /tools/bin/grub-\*]{.mark}

[/tools/bin/grub-editenv /tools/bin/grub-kbdcomp
/tools/bin/grub-mknetdir /tools/bin/grub-mkstandalone]{.mark}

[/tools/bin/grub-file /tools/bin/grub-menulst2cfg
/tools/bin/grub-mkpasswd-pbkdf2 /tools/bin/grub-render-label]{.mark}

[/tools/bin/grub-fstest /tools/bin/grub-mkimage
/tools/bin/grub-mkrelpath /tools/bin/grub-script-check]{.mark}

[/tools/bin/grub-glue-efi /tools/bin/grub-mklayout
/tools/bin/grub-mkrescue /tools/bin/grub-syslinux2cfg]{.mark}

[注意，编译完成后，应该进行make
check,检查编译是否正确。实际测试结果惨不忍睹。============================================================================]{.mark}

[Testsuite summary for GRUB 2.12]{.mark}
...
[基本上，不能使用。为了加快进度，使用上游grub替代。]{.mark}
...
[ERROR: erofs_test]{.mark}

[ERROR: ext234_test]{.mark}

[PASS: squashfs_test]{.mark}

[PASS: iso9660_test]{.mark}

[ERROR: hfsplus_test]{.mark}

[ERROR: ntfs_test]{.mark}

[ERROR: reiserfs_test]{.mark}

[ERROR: fat_test]{.mark}

[ERROR: minixfs_test]{.mark}

[ERROR: xfs_test]{.mark}

[ERROR: f2fs_test]{.mark}

[ERROR: nilfs2_test]{.mark}

[ERROR: romfs_test]{.mark}

[ERROR: exfat_test]{.mark}
...
[ERROR: btrfs_test]{.mark}

[ERROR: zfs_test]{.mark}

[ERROR: zfs_zstd_test]{.mark}

[PASS: cpio_test]{.mark}

[PASS: example_scripted_test]{.mark}

[PASS: gettext_strings_test]{.mark}

[PASS: grub_script_blanklines]{.mark}

[PASS: grub_script_final_semicolon]{.mark}

[PASS: grub_script_dollar]{.mark}

[PASS: grub_script_no_commands]{.mark}

[PASS: syslinux_test]{.mark}

[ERROR: luks1_test]{.mark}

[ERROR: luks2_test]{.mark}
...
[FAIL: example_grub_script_test]{.mark}

[FAIL: grub_script_eval]{.mark}

[FAIL: grub_script_test]{.mark}

[FAIL: grub_script_echo1]{.mark}

[FAIL: grub_script_leading_whitespace]{.mark}

[FAIL: grub_script_echo_keywords]{.mark}

[FAIL: grub_script_vars1]{.mark}

[FAIL: grub_script_for1]{.mark}

[FAIL: grub_script_while1]{.mark}

[FAIL: grub_script_if]{.mark}

[FAIL: grub_script_comments]{.mark}

[FAIL: grub_script_functions]{.mark}

[FAIL: grub_script_break]{.mark}

[FAIL: grub_script_continue]{.mark}

[FAIL: grub_script_shift]{.mark}

[FAIL: grub_script_blockarg]{.mark}

[FAIL: grub_script_setparams]{.mark}

[FAIL: grub_script_return]{.mark}

[ERROR: grub_cmd_cryptomount]{.mark}

[FAIL: grub_cmd_regexp]{.mark}

[FAIL: grub_cmd_date]{.mark}

[FAIL: grub_cmd_set_date]{.mark}

[FAIL: grub_cmd_sleep]{.mark}

[PASS: grub_script_expansion]{.mark}

[FAIL: grub_script_not]{.mark}

[PASS: partmap_test]{.mark}

[FAIL: hddboot_test]{.mark}

[SKIP: fddboot_test]{.mark}

[FAIL: cdboot_test]{.mark}

[SKIP: netboot_test]{.mark}

[ERROR: serial_test]{.mark}

[SKIP: pseries_test]{.mark}

[FAIL: core_compress_test]{.mark}

[FAIL: xzcompress_test]{.mark}

[FAIL: gzcompress_test]{.mark}

[FAIL: lzocompress_test]{.mark}

[FAIL: grub_cmd_echo]{.mark}

[FAIL: help_test]{.mark}

[FAIL: grub_script_gettext]{.mark}

[FAIL: grub_script_escape_comma]{.mark}

[FAIL: grub_script_strcmp]{.mark}

[PASS: test_sha512sum]{.mark}

[FAIL: test_unset]{.mark}

[ERROR: grub_func_test]{.mark}

[FAIL: grub_cmd_tr]{.mark}

[FAIL: file_filter_test]{.mark}

[PASS: grub_cmd_test]{.mark}

[FAIL: asn1_test]{.mark}

[SKIP: tpm2_key_protector_test]{.mark}

[PASS: example_unit_test]{.mark}

[PASS: printf_test]{.mark}

[PASS: date_test]{.mark}

[PASS: priority_queue_unit_test]{.mark}

[PASS: cmp_test]{.mark}

[============================================================================]{.mark}

[Testsuite summary for GRUB 2.14]{.mark}
...
[See ./test-suite.log]{.mark}

[Please report to bug-grub@gnu.org]{.mark}

[============================================================================]{.mark}

[make\[4\]: \*\*\* \[Makefile:12898: test-suite.log\] Error 1]{.mark}

[make\[4\]: Leaving directory
\'/home/rocky/rpmbuild/BUILD/grub-2.14/cross-build\']{.mark}

[make\[3\]: \*\*\* \[Makefile:13006: check-TESTS\] Error 2]{.mark}

[make\[3\]: Leaving directory
\'/home/rocky/rpmbuild/BUILD/grub-2.14/cross-build\']{.mark}

[make\[2\]: \*\*\* \[Makefile:13866: check-am\] Error 2]{.mark}

[make\[2\]: Leaving directory
\'/home/rocky/rpmbuild/BUILD/grub-2.14/cross-build\']{.mark}

[make\[1\]: \*\*\* \[Makefile:12783: check-recursive\] Error 1]{.mark}

[make\[1\]: Leaving directory
\'/home/rocky/rpmbuild/BUILD/grub-2.14/cross-build\']{.mark}

[make: \*\*\* \[Makefile:13868: check\] Error 2]{.mark}

## wget {#wget .unnumbered}
...
| (/home/r                                                              |
| ocky/rpmbuild/BUILD/rust-1.88.0-build/rustc-1.88.0-src/src/bootstrap) |
...
添加export RUSTFLAGS=\"-A dead_code\"，可以规避上述问题。
...
不做了，现在编译需要补充很多包，比如llvm等，成本过高，后面再编译了。
...
## dracut {#dracut .unnumbered}

# 配置临时系统 {#配置临时系统 .unnumbered}
...
## 作为容器，启动临时系统 {#作为容器启动临时系统 .unnumbered}
...
\--bind=/run/dbus是为了让logind.service能够访问dbus。

## 作为虚拟机，启动临时系统 {#作为虚拟机启动临时系统 .unnumbered}
...
qemu-img create -f qcow2 rocky_vm.qcow2 10G
...
在./grub-core/kern/loongarch64/dl.c文件中，不存在grub_arch_dl_min_alignment函数。
...
sudo systemd-nspawn -b -D ./rocky_vm \--private-users=1
\--bind=/run/dbus
...
不使用rocky Linux自带的grub源码，改用一个可用的grub源码。

方案4：

直接使用Debian 提供的grub，实现启动。先用这个方案，再改成方案3。
...
又遇到了新的问题，卡住了，书本里的内容已经不足以解决我的问题了。

现在我已经一只脚踏入了无人区。我太难了。

mount 程序的使用。

要使/etc/fstab生效，mount
程序必须能够正常工作。而mount程序正常工作的前提之一，是所有者必须是root。

此外，需要将CONFIG_NLS_CODEPAGE_437设置为y,以将nls_cp437模块内置。
...
在宿主机里 ssh <rocky@192.168.122.228>
...
今天就搞到这里了！终于可以进展到下一步了！

## 临时系统补全 {#临时系统补全 .unnumbered}

### Perl {#perl .unnumbered}
...
经过对比，/tools/lib64/gcc/loongarch64-unknown-linux-gnu/14.3.1/include/limits.h文件，与Debian
系统提供的/usr/lib/gcc/loongarch64-linux-gnu/14/include/limits.h文件确实存在不同。用Debian
提供的版本，看看是否能够修复bug。
...
### Grep {#grep-1 .unnumbered}

grep的编译，需要用到rpm命令。确保/tools/lib/rpm中的macros的修改是正确的。包括对/tools的替换，对sysconfdir,
localstatedir, var, sharedstatedir的设置等。
...
### Procps-ng {#procps-ng .unnumbered}
...
### help2man {#help2man .unnumbered}

perl相关的模块还没有安装，所以编译的时候先\--disable-nls

### debugedit {#debugedit .unnumbered}

依赖help2man。注释掉gpgverify语句

## ~~gpg2~~ {#gpg2 .unnumbered}

~~rpm包中使用的gpgverify脚本会调用gpg2。依赖的内容太多了，暂时不管它了。~~

### libkcapi {#libkcapi .unnumbered}

libxcrypt依赖fipscheck,
这个包已经由libkcapi提供。所以，需要编译libkcapi。

### python-setuptools {#python-setuptools .unnumbered}

## 不能直接python setup.py install, 这样不会自动调用shim,无法直接import distutils。 {#不能直接python-setup.py-install-这样不会自动调用shim无法直接import-distutils .unnumbered}

最终使用的是这个方案：

## python3 -m build \--wheel {#python3--m-build---wheel .unnumbered}

##  sudo pip3 install dist/setuptools-69.0.3-py3-none-any.whl {#sudo-pip3-install-distsetuptools-69.0.3-py3-none-any.whl .unnumbered}

## rpm包的编译 {#rpm包的编译 .unnumbered}

### redhat-rpm\--config {#redhat-rpm--config .unnumbered}

修改\~/rpmbuild/SOURCES/rpmrc，添加loongarch的optflags：

optflags:loongarch64 %{\_\_global_compiler_flags} -march=loongarch64
-msimd=lsx
...
rpmbuild \--rebuild \~/rpmbuild/SRPMS/redhat-rpm-config-293-2.src.rpm
\--nodeps
...
### python-srpm-macros {#python-srpm-macros .unnumbered}

### 由于：%%\_\_os_install_post_python is defined in python-srpm-macros and contains several policies，很多rpm包需要这个宏。编译安装： {#由于__os_install_post_python-is-defined-in-python-srpm-macros-and-contains-several-policies很多rpm包需要这个宏编译安装 .unnumbered}

### rpmbuild \--rebuild /opt/srpms/Packages/python-rpm-macros-3.12-10.el10.src.rpm {#rpmbuild---rebuild-optsrpmspackagespython-rpm-macros-3.12-10.el10.src.rpm .unnumbered}
...
经过测试，rpm不会从/usr/lib/rpm/macros.d文件夹中加载宏。需要调整。

是这样的，rpm安装在tools，它会从/tools/lib/rpm/macros.d中加载宏。

解决方案：将/usr/lib/rpm/macrod.d中的内容，符号链接到/tools/lib/rpm/macros.d中。

### 发行版信息包 {#发行版信息包 .unnumbered}

找不到类似fedora-repos的包。

### rocky-release {#rocky-release .unnumbered}
...
### iso-codes {#iso-codes .unnumbered}
...
sudo rpm -ivh
\~/rpmbuild/RPMS/noarch/setup-2.14.5-7.el10.loongarch.noarch.rpm

未改动。

### Filesystem {#filesystem .unnumbered}
...
### Basesystem {#basesystem .unnumbered}

rpmbuild \--rebuild /opt/srpms/Packages/basesystem-11-22.el10.src.rpm

sudo rpm -ivh
\~/rpmbuild/RPMS/noarch/basesystem-11-22.el10.loongarch.noarch.rpm

未进行任何改动

### less {#less .unnumbered}
...
sudo rpm -Uvh
/home/rocky/rpmbuild/RPMS/loongarch64/less-661-3.el10.loongarch.loongarch64.rpm
\--nodeps

### rsync {#rsync .unnumbered}

直接编译内核头文件的时候，依赖rsync。尝试编译rpm包。生成了5个包：

rsync-rrsync-3.4.1-2.el10.loongarch.noarch.rpm

rsync-daemon-3.4.1-2.el10.loongarch.noarch.rpm

rsync-3.4.1-2.el10.loongarch.loongarch64.rpm

rsync-debuginfo-3.4.1-2.el10.loongarch.loongarch64.rpm

rsync-debugsource-3.4.1-2.el10.loongarch.loongarch64.rpm

只安装其中一个就可以了。

### 内核头文件 kernel-header {#内核头文件-kernel-header .unnumbered}
...
sudo rpm -Uvh
/home/rocky/rpmbuild/RPMS/loongarch64/kernel-headers-6.12.0-124.21.1.el10.loongarch.loongarch64.rpm

### glibc {#glibc-1 .unnumbered}
...
### Zlib {#zlib .unnumbered}

正常编译即可，不需要调整spec文件。

安装

sudo rpm -Uvh \$(ls \~/rpmbuild/RPMS/loongarch64/{minizip,zlib}-\*
\|grep -v -e \"debug\") \--nodeps

### libxcrypt {#libxcrypt-1 .unnumbered}
...
rpmbuild -bb \~/rpmbuild/SPECS/libxcrypt.spec \--nodeps \--with
bootstrap \--without new_api \--nocheck \--without compat_pkg \--without
staticlib

由于我添加了\--without compat_pkg \--without
staticlib，编译生成的包少了很多，只有四个：
...
### binutils {#binutils-1 .unnumbered}
...
rpmbuild \--rebuild
/home/rocky/rpmbuild/SRPMS/binutils-2.41-58.el10.loongarch.2.src.rpm
\--with bootstrap \--nodeps \--nocheck \--without debuginfod
...
binutils-devel-2.41-58.el10.loongarch.2.loongarch64.rpm \--nodeps

由于没有/usr/sbin/alternatives， 需要手动设置符号连接：

sudo ln -sv ld.bfd /bin/ld

### GMP {#gmp-2 .unnumbered}
...
rpmbuild \--rebuild
/home/rocky/rpmbuild/SRPMS/gmp-6.2.1-12.el10.loongarch.1.src.rpm
\--nodeps

生成了以下7个文件：

gmp-c++-debuginfo-6.2.1-12.el10.loongarch.loongarch64.rpm

gmp-c++-6.2.1-12.el10.loongarch.loongarch64.rpm

gmp-devel-6.2.1-12.el10.loongarch.loongarch64.rpm

gmp-6.2.1-12.el10.loongarch.loongarch64.rpm

gmp-static-6.2.1-12.el10.loongarch.loongarch64.rpm

gmp-debuginfo-6.2.1-12.el10.loongarch.loongarch64.rpm

gmp-debugsource-6.2.1-12.el10.loongarch.loongarch64.rpm
...
gmp-c++-6.2.1-12.el10.loongarch.loongarch64.rpm
gmp-devel-6.2.1-12.el10.loongarch.loongarch64.rpm
gmp-static-6.2.1-12.el10.loongarch.loongarch64.rpm \--nodeps

### MPFR {#mpfr-2 .unnumbered}

不需要特殊处理，直接编译即可。

生成了5个包：

mpfr-devel-4.2.1-5.el10.loongarch.loongarch64.rpm

mpfr-4.2.1-5.el10.loongarch.loongarch64.rpm

mpfr-doc-4.2.1-5.el10.loongarch.noarch.rpm

mpfr-debuginfo-4.2.1-5.el10.loongarch.loongarch64.rpm

mpfr-debugsource-4.2.1-5.el10.loongarch.loongarch64.rpm

### libMPC {#libmpc-1 .unnumbered}

直接编译，生成了以下的rpm包

libmpc-devel-1.3.1-7.el10.loongarch.loongarch64.rpm

libmpc-doc-1.3.1-7.el10.loongarch.noarch.rpm

libmpc-1.3.1-7.el10.loongarch.loongarch64.rpm

libmpc-debugsource-1.3.1-7.el10.loongarch.loongarch64.rpm

libmpc-debuginfo-1.3.1-7.el10.loongarch.loongarch64.rpm

安装：

sudo rpm -ihv libmpc-devel-1.3.1-7.el10.loongarch.loongarch64.rpm
libmpc-1.3.1-7.el10.loongarch.loongarch64.rpm
../noarch/libmpc-doc-1.3.1-7.el10.loongarch.noarch.rpm \--nodeps
...
xz-debuginfo-5.6.2-4.el10.loongarch.loongarch64.rpm

xz-devel-5.6.2-4.el10.loongarch.loongarch64.rpm

xz-libs-5.6.2-4.el10.loongarch.loongarch64.rpm

xz-lzma-compat-5.6.2-4.el10.loongarch.loongarch64.rpm

xz-lzma-compat-debuginfo-5.6.2-4.el10.loongarch.loongarch64.rpm

xz-static-5.6.2-4.el10.loongarch.loongarch64.rpm

xz-libs-debuginfo-5.6.2-4.el10.loongarch.loongarch64.rpm

xz-5.6.2-4.el10.loongarch.loongarch64.rpm

xz-debugsource-5.6.2-4.el10.loongarch.loongarch64.rpm
...
xz-lzma-compat-5.6.2-4.el10.loongarch.loongarch64.rpm
xz-static-5.6.2-4.el10.loongarch.loongarch64.rpm
xz-5.6.2-4.el10.loongarch.loongarch64.rpm \--nodeps
...
注意：安装了meson以后，会在/usr/lib/rpm/macros.d
内创建文件macros.meson。但系统中的rpm依然是/tools/中的，所以需要做一个符号链接。

注意：meson是一个python的程序，所以需要注意其安装位置。目前系统的python依然是在/tools/中，所以必要的时候需要做符号链接！

注意：我又用pip 装了一个meson.最终生效的，可能是这个meson.

version_no_tilde 这个宏，应该是定义在 rust-srpm-macros中的。

### ninja-build {#ninja-build .unnumbered}

meson运行的时候，依赖ninja-build

rpmbuild \--rebuild
/opt/srpms/Packages/ninja-build-1.11.1-9.el10.src.rpm \--nodeps \--with
bootstrap

### LZ4 {#lz4-1 .unnumbered}

编译需要meson，需要想办法把meson补全了。

meson依赖python-setuptools

编译

lz4-libs-1.9.4-8.el10.loongarch.loongarch64.rpm

lz4-1.9.4-8.el10.loongarch.loongarch64.rpm

lz4-devel-1.9.4-8.el10.loongarch.loongarch64.rpm

lz4-static-1.9.4-8.el10.loongarch.loongarch64.rpm

lz4-debugsource-1.9.4-8.el10.loongarch.loongarch64.rpm

lz4-libs-debuginfo-1.9.4-8.el10.loongarch.loongarch64.rpm

lz4-debuginfo-1.9.4-8.el10.loongarch.loongarch64.rpm

安装不含debug的4个包。

### ZSTD {#zstd-1 .unnumbered}
...
libzstd-1.5.5-9.el10.loongarch.loongarch64.rpm
...
zstd-debugsource-1.5.5-9.el10.loongarch.loongarch64.rpm

libzstd-debuginfo-1.5.5-9.el10.loongarch.loongarch64.rpm

zstd-debuginfo-1.5.5-9.el10.loongarch.loongarch64.rpm

sudo rpm -ivh libzstd-1.5.5-9.el10.loongarch.loongarch64.rpm
libzstd-devel-1.5.5-9.el10.loongarch.loongarch64.rpm
zstd-1.5.5-9.el10.loongarch.loongarch64.rpm
libzstd-static-1.5.5-9.el10.loongarch.loongarch64.rpm \--nodeps

### GCC {#gcc-1 .unnumbered}
...
\+ cp -pr
\'/home/rocky/rpmbuild/BUILD/gcc-14.3.1-20250617/rpm.doc/libgccjit-devel/\*\'
/home/rocky/rpmbuild/BUILDROOT/gcc-14.3.1-2.1.el10.loongarch.1.loongarch64/usr/share/doc/libgccjit-devel
...
\+ cp -pr
/home/rocky/rpmbuild/BUILD/gcc-14.3.1-20250617/gcc/jit/docs/examples
/home/rocky/rpmbuild/BUILDROOT/gcc-14.3.1-2.1.el10.loongarch.1.loongarch64/usr/share/doc/libgccjit-devel
...
+-----------------------------------------------------------------------+
| gcc-debugsource-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm           |

| cpp-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm             |

| gcc-gfortran-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm    |

| gcc-c++-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm         |

| libgccjit-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm       |

| gcc-c++-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm                   |

| gcc-gfortran-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm              |

| libgccjit-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm                 |

| gcc-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm                       |

| libasan-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm         |

| libstdc++-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm       |

| libstdc++-devel-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm           |

| libstdc++-static-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm          |

| libtsan-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm         |

| gcc-plugin-devel-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm          |

| libtsan-static-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm            |

| libasan-static-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm            |

| libgfortran-static-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm        |

| libgfortran-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm     |

| liblsan-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm         |

| gcc-gdb-plugin-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm  |

| libubsan-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm        |

| cpp-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm                       |

| libstdc++-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm                 |

| libgcc-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm          |

| libubsan-static-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm           |

| libasan-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm                   |

| liblsan-static-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm            |

| libitm-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm          |

| g                                                                     |
| cc-plugin-devel-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm |

| libtsan-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm                   |

| libgomp-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm         |

| libgfortran-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm               |

| libubsan-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm                  |

| libgomp-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm                   |

| liblsan-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm                   |

| gcc-gdb-plugin-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm            |

| libatomic-static-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm          |

| libgcc-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm                    |

| libatomic-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm                 |

| libitm-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm                    |

| libgccjit-devel-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm           |

| libitm-devel-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm              |

| libitm-static-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm             |

| libatomic-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm       |

| gcc-debuginfo-14.3.1-2.1.el10.loongarch.1.loongarch64.rpm             |
+-----------------------------------------------------------------------+
...
| /u                                                                    |
| sr/lib/gcc/loongarch64-redhat-linux/14/install-tools/include/limits.h |
...
| /us                                                                   |
| r/libexec/gcc/loongarch64-redhat-linux/14/install-tools/mkinstalldirs |
...
### 重新编译工具链 {#重新编译工具链 .unnumbered}

#### glibc {#glibc-2 .unnumbered}

glibc的spec文件做过改动，不能直接从源码编译。用之前写的编译glibc的脚本编译。

确保存在/usr/lib/rpm/redhat/redhat-annobin-cc1文件，且这个文件为空。因为现在的GCC并不支持annbin。

编译成功，安装软件包，一共442个包。好多包。

## binutils {#binutils-2 .unnumbered}
...
#### ZLIB {#zlib-1 .unnumbered}

这个包的源码没有进行调整，直接使用原始的src.rpm包编译安装即可。

rpmbuild \--rebuild /opt/srpms/Packages/zlib-1.2.11-40.el9.src.rpm
\--nodeps

#### libxcrypt {#libxcrypt-2 .unnumbered}

使用之前留下来的spec文件来编译。

注意，还是需要modprobe很多包。

#### GMP {#gmp-3 .unnumbered}

正常操作即可。

#### MPFR {#mpfr-3 .unnumbered}

正常操作

#### LibMPC {#libmpc-2 .unnumbered}
...
#### LZ4 {#lz4-2 .unnumbered}

正常操作。

#### ZSTD {#zstd-2 .unnumbered}

正常操作。

### GCC {#gcc-2 .unnumbered}
...
在/tools/lib/rpm/platform/loongarch64-linux/macros结尾，添加：

%with_bootstrap 1

## 系统交互环境 {#系统交互环境 .unnumbered}

### ncurses {#ncurses-1 .unnumbered}

注释掉SPEC文件中的gpgverify 行

生成了如下的11个文件：

ncurses-debuginfo-6.4-14.20240127.el10\~bootstrap.loongarch.loongarch64.rpm

ncurses-c++-libs-debuginfo-6.4-14.20240127.el10\~bootstrap.loongarch.loongarch64.rpm

ncurses-libs-6.4-14.20240127.el10\~bootstrap.loongarch.loongarch64.rpm

ncurses-6.4-14.20240127.el10\~bootstrap.loongarch.loongarch64.rpm

ncurses-base-6.4-14.20240127.el10\~bootstrap.loongarch.noarch.rpm

ncurses-devel-6.4-14.20240127.el10\~bootstrap.loongarch.loongarch64.rpm

ncurses-c++-libs-6.4-14.20240127.el10\~bootstrap.loongarch.loongarch64.rpm

ncurses-term-6.4-14.20240127.el10\~bootstrap.loongarch.noarch.rpm

ncurses-libs-debuginfo-6.4-14.20240127.el10\~bootstrap.loongarch.loongarch64.rpm

ncurses-debugsource-6.4-14.20240127.el10\~bootstrap.loongarch.loongarch64.rpm

ncurses-static-6.4-14.20240127.el10\~bootstrap.loongarch.loongarch64.rpm

安装其中与debug无关的7个。

### readline {#readline-1 .unnumbered}

正常编译即可。

### BASH {#bash-1 .unnumbered}

也需要注释掉gpgverify语句

编译生成5个文件。

bash-devel-5.2.26-6.el10\~bootstrap.loongarch.loongarch64.rpm

bash-debuginfo-5.2.26-6.el10\~bootstrap.loongarch.loongarch64.rpm

bash-debugsource-5.2.26-6.el10\~bootstrap.loongarch.loongarch64.rpm

bash-doc-5.2.26-6.el10\~bootstrap.loongarch.loongarch64.rpm

bash-5.2.26-6.el10\~bootstrap.loongarch.loongarch64.rpm

安装其中3个。

### bash-completion {#bash-completion .unnumbered}

正常编译安装即可。

### bzip2 {#bzip2-1 .unnumbered}

注释掉gpgverify

编译以后，生成了如下7个文件：

bzip2-1.0.8-25.el10\~bootstrap.loongarch.loongarch64.rpm

bzip2-debuginfo-1.0.8-25.el10\~bootstrap.loongarch.loongarch64.rpm

bzip2-static-1.0.8-25.el10\~bootstrap.loongarch.loongarch64.rpm

bzip2-devel-1.0.8-25.el10\~bootstrap.loongarch.loongarch64.rpm

bzip2-libs-1.0.8-25.el10\~bootstrap.loongarch.loongarch64.rpm

bzip2-debugsource-1.0.8-25.el10\~bootstrap.loongarch.loongarch64.rpm

bzip2-libs-debuginfo-1.0.8-25.el10\~bootstrap.loongarch.loongarch64.rpm

安装其中与debug无关的4个。

### TCL {#tcl-1 .unnumbered}

直接编译，生成了如下的文件：

tcl-devel-8.6.13-4.el10\~bootstrap.loongarch.loongarch64.rpm

tcl-doc-8.6.13-4.el10\~bootstrap.loongarch.noarch.rpm

tcl-8.6.13-4.el10\~bootstrap.loongarch.loongarch64.rpm

tcl-debuginfo-8.6.13-4.el10\~bootstrap.loongarch.loongarch64.rpm

tcl-debugsource-8.6.13-4.el10\~bootstrap.loongarch.loongarch64.rpm

### libdb {#libdb-1 .unnumbered}

不需要了。

### gdbm {#gdbm .unnumbered}

正常编译，生成6个文件：

gdbm-devel-1.23-13.el10\~bootstrap.loongarch.loongarch64.rpm

gdbm-libs-1.23-13.el10\~bootstrap.loongarch.loongarch64.rpm

gdbm-libs-debuginfo-1.23-13.el10\~bootstrap.loongarch.loongarch64.rpm

gdbm-1.23-13.el10\~bootstrap.loongarch.loongarch64.rpm

gdbm-debuginfo-1.23-13.el10\~bootstrap.loongarch.loongarch64.rpm

gdbm-debugsource-1.23-13.el10\~bootstrap.loongarch.loongarch64.rpm

### Perl {#perl-1 .unnumbered}

check通不过，编译的时候添加\--nocheck

生成了非常229个的文件，安装了其中188个。

调整rpm的宏中关于perl的部分。

### Perl-srpm-macros {#perl-srpm-macros .unnumbered}

包的构建和安装非常简单。生成了1个包：

perl-srpm-macros-1-57.el10\~bootstrap.loongarch.noarch.rpm。

### Perl-Fedora-VSP {#perl-fedora-vsp .unnumbered}

正常构建即可。

### Perl-generators {#perl-generators .unnumbered}

正常构建即可。

## Python-rpm-macros {#python-rpm-macros .unnumbered}

正常编译，安装。

复制文件到/tools/lib/rpm/macros.d/

### Unzip {#unzip .unnumbered}
...
### Autoconf {#autoconf-1 .unnumbered}

rpmbuild \--rebuild /opt/srpms/Packages/autoconf-2.71-13.el10.src.rpm
\--without autoconf_enables_emacs \--nodeps \--nocheck

### Automake {#automake-1 .unnumbered}

rpmbuild \--rebuild /opt/srpms/Packages/automake-1.16.5-20.el10.src.rpm
\--without check \--nodeps

### Texinfo {#texinfo .unnumbered}

修改spec文件

生成5个文件，安装其中的3个。

### libtool {#libtool-1 .unnumbered}

libtool-ltdl-2.4.7-13.el10\~bootstrap.loongarch.loongarch64.rpm

libtool-debugsource-2.4.7-13.el10\~bootstrap.loongarch.loongarch64.rpm

libtool-ltdl-debuginfo-2.4.7-13.el10\~bootstrap.loongarch.loongarch64.rpm

libtool-ltdl-devel-2.4.7-13.el10\~bootstrap.loongarch.loongarch64.rpm

libtool-2.4.7-13.el10\~bootstrap.loongarch.loongarch64.rpm

### Pkgconf {#pkgconf-2 .unnumbered}

生成了10个rpm包。

rpmbuild \--rebuild /opt/srpms/Packages/pkgconf-2.1.0-3.el10.src.rpm
\--nocheck \--nodeps

### Lua {#lua-1 .unnumbered}

正常编译即可。

### diffutils {#diffutils-1 .unnumbered}

正常编译即可。

### attr {#attr-1 .unnumbered}

正常编译即可。

### acl {#acl-1 .unnumbered}

正常编译即可。

### tar {#tar-1 .unnumbered}

rpmbuild \--rebuild /opt/srpms/Packages/tar-1.35-9.el10_1.src.rpm
\--without check \--without selinux \--nodeps

### cpio {#cpio-1 .unnumbered}

### bison {#bison-1 .unnumbered}

测试时间确实很长。

###  FLEX {#flex-1 .unnumbered}
...
### patch {#patch-1 .unnumbered}

修改spec文件中关于selinux补丁的部分，不打这个补丁。

### Check {#check-1 .unnumbered}

需要依赖cmake来编译。先跳过。

### CMake {#cmake .unnumbered}

尝试一下，强行编译cmake能否通过。

依赖openssl。先跳过，编译完openssl再来编译cmake.

### make {#make-1 .unnumbered}

正常编译。

### tcsh {#tcsh-1 .unnumbered}

不要check

### sed {#sed-1 .unnumbered}

正常编译即可。

### grep {#grep-2 .unnumbered}

正常编译即可。

### findutils {#findutils-1 .unnumbered}

正常编译即可。

### file {#file-1 .unnumbered}

rpmbuild -bb \~/rpmbuild/SPECS/file.spec \--nodeps \--without python3

### NSPR {#nspr-1 .unnumbered}

nspr源码在NSS中，不再单独编译这个了。

### sqlite {#sqlite-1 .unnumbered}

测试需要花的时间比较长。

### pcre2 {#pcre2-1 .unnumbered}
...
### libsepol {#libsepol-1 .unnumbered}

正常编译即可。

### libgpg-error {#libgpg-error .unnumbered}

正常编译即可。

### libgcrypt {#libgcrypt .unnumbered}
...
check需要annocheck命令，没有安装，因此跳过check。

### chrpath {#chrpath .unnumbered}

正常编译即可。

### expect {#expect .unnumbered}

需要依赖chrpath。已经安装。

### screen {#screen .unnumbered}

没有找到安装包。

### dejagnu {#dejagnu .unnumbered}

正常编译即可。

### libffi {#libffi .unnumbered}
...
遇到了一个编译的错误，../src/loongarch64/ffi.c:525:7: error: implicit
declaration of
...
### lksctp-tools {#lksctp-tools .unnumbered}

完成

### openssl {#openssl-1 .unnumbered}
...
\+ rename so.3 so.3.5.1
\'/home/rocky/rpmbuild/BUILDROOT/openssl-3.5.1-4.el10\~bootstrap.loongarch.1.loongarch64/usr/lib64/\*.so.3\'
...
因为so文件最后安装到了/usr/lib/。我应该想办法把_lib设置为/usr/lib64/
...
### libxml2 {#libxml2 .unnumbered}

修改spec文件，去除\_\_pycache目录相关的处理和打包。

需要设置一下PKG_CONFIG_PATH环境变量。

编译的时候遇到了这样的报错：

tree.c:227:17: error: \'SIZE_MAX\' undeclared (first use in this
function)
...
tree.c:49:1: note: \'SIZE_MAX\' is defined in header \'\<stdint.h\>\';
this is probably fixable by adding \'#include \<stdint.h\>\'
...
跟大模型学会了下载patch文件的办法：直接下载https://gitlab.gnome.org/GNOME/libxml2/-/commit/3773bb3f89426297dd82d8ad5998059898d5172a.patch
即可获得补丁。
...
ERROR 0002: file \'/usr/lib64/libxml2.so.2.12.5\' contains an invalid
rpath \'/tools/lib64\' in \[/tools/lib64\]

看来之前的设置PKG_CONFIG_PATH不太合理

按照程序的提示，添加QA_RPATHS=\$(( 0x0002\|0x0004 ))

### libxslt {#libxslt .unnumbered}

正常编译即可。

### sgml-common {#sgml-common .unnumbered}

正常编译即可。

### docbook-dtds {#docbook-dtds .unnumbered}

正常编译即可。

### docbook-style-xls {#docbook-style-xls .unnumbered}

正常编译即可。

### xmlto {#xmlto .unnumbered}

正常编译即可。

### expat {#expat-1 .unnumbered}

正常编译即可。

### mpdecimal {#mpdecimal .unnumbered}

正常编译即可。

### python3 {#python3 .unnumbered}

依赖mpdecimal-devel，需要先安装这个。

按照书中的要求进行修改。

### libxml2 {#libxml2-1 .unnumbered}

解决以来关系以后，重新编译libxml2

### NSS {#nss-1 .unnumbered}

按照书中的提示，修改spec文件，注释掉一部分代码。

config.sub以及config.guess太老，需要更新。

使用autoreconf -ifv更新它们。

### PCRE2 {#pcre2-2 .unnumbered}
...
暂时给rpmbuild添加一些参数，规避这个问题

\--define \'\_unpackaged_files_terminate_build 0\'

### Cmake {#cmake-1 .unnumbered}
...
### Hostname {#hostname .unnumbered}

正常编译即可。

### Coreutils {#coreutils-1 .unnumbered}

注释掉spec中的gpg校验部分。有两个案例会fail， 不管了。加上nocheck。

coreutils-single和coreutils两个包冲突，前者不装了。

### Python-rpm-generators {#python-rpm-generators .unnumbered}

正常编译安装即可。还依赖了python-packaging，
pyproject-rpm-macros，pyproject-srpm-macros,
python-packaging，都需要编译安装。

### Python-setuptools {#python-setuptools-1 .unnumbered}

依赖python-rpm-generators。

### Re2c {#re2c .unnumbered}

没有找到源码，不知道它

### Ninja-build {#ninja-build-1 .unnumbered}

正常编译即可。不需要修正rpmmacrodir，因为现在的spec文件中已经不用这个宏了。

### Meson {#meson .unnumbered}
...
### Dos2unix {#dos2unix .unnumbered}

注释掉spec文件中，关于gpgverify的语句。

### Swig {#swig .unnumbered}

添加\--without testsuite参数

### Less {#less-1 .unnumbered}

正常编译即可。

### Gzip {#gzip-1 .unnumbered}

正常编译。

### libpaper {#libpaper .unnumbered}

正常编译。

### chrpath {#chrpath-1 .unnumbered}
...
### Psutils {#psutils .unnumbered}

测试通不过。加上\--nocheck。

### libedit {#libedit .unnumbered}

正常编译即可。

### multilib-rpm-config {#multilib-rpm-config .unnumbered}

正常编译即可。

### llvm {#llvm .unnumbered}
...
### python-lit {#python-lit .unnumbered}

找不到源码包了，无法安装。

### clang {#clang .unnumbered}

[和llvm相关，先跳过。]{.mark}

### ccache {#ccache .unnumbered}

找不到源码包，无法安装。

### nasm {#nasm .unnumbered}

正常编译即可。

### Gpref {#gpref .unnumbered}

正常编译即可。

### libutempter {#libutempter .unnumbered}

正常编译即可。

### libpipeline {#libpipeline .unnumbered}

正常编译即可。

## 图形相关软件包 {#图形相关软件包 .unnumbered}

### giflib {#giflib .unnumbered}

这个包依赖mingw64-gcc, mingw64-filesystem

做一个完全不依赖mingw的版本。

### libpng {#libpng .unnumbered}

正常编译即可。

### libjpeg-turbo {#libjpeg-turbo .unnumbered}

正常编译即可。

### Jbigkit {#jbigkit .unnumbered}

正常编译即可。

### libtiff {#libtiff .unnumbered}

正常编译即可。

### lcms2 {#lcms2 .unnumbered}

正常编译即可。

### libmng {#libmng .unnumbered}

正常编译即可。

### Xorg-X11-utils-macros {#xorg-x11-utils-macros .unnumbered}

正常编译即可。

### Xorg-X11-Proto-devel {#xorg-x11-proto-devel .unnumbered}

正常编译即可。

### libXau {#libxau .unnumbered}

正常编译即可。

### libXdmcp {#libxdmcp .unnumbered}

正常编译即可。

### xcb-proto {#xcb-proto .unnumbered}

正常编译即可。

### libxcb {#libxcb .unnumbered}

正常编译即可。

### xorg-x11-xtrans-devel {#xorg-x11-xtrans-devel .unnumbered}

正常编译即可。

### libx11 {#libx11 .unnumbered}

正常编译即可。

### brotli {#brotli .unnumbered}

正常编译即可。

### freetype {#freetype .unnumbered}

依赖brotli

### forge-srpm-macros {#forge-srpm-macros .unnumbered}
...
### fonts-rpm-macros {#fonts-rpm-macros .unnumbered}

依赖forge-srpm-macros，顺便安装了。

把安装的宏定义文件，复制到/tools/lib/rpm/macrod.d/

### fontconfig {#fontconfig .unnumbered}

按照书中操作即可。

### Pixman {#pixman .unnumbered}

正常编译即可。

### libimagequant {#libimagequant .unnumbered}

找不到源码。

### libICE {#libice .unnumbered}

正常编译即可。

### libSM {#libsm .unnumbered}

修改spec文件，将\--with-libuuid, 改为\--without-libuuid

### libXrender {#libxrender .unnumbered}

正常编译即可。

### libXext {#libxext .unnumbered}

正常编译即可。

### libXt {#libxt .unnumbered}

正常编译即可。

### libXpm {#libxpm .unnumbered}

正常编译即可。

### libXmu {#libxmu .unnumbered}

正常编译即可。

### libXaw {#libxaw .unnumbered}

正常编译即可。

### libXfixes {#libxfixes .unnumbered}

正常编译即可。

### libXi {#libxi .unnumbered}

正常编译即可。

### libXft {#libxft .unnumbered}

正常编译即可。

### libXinerama {#libxinerama .unnumbered}

正常编译即可。

### libXtst {#libxtst .unnumbered}

正常编译即可。

### python-wheels {#python-wheels .unnumbered}

正常编译即可。

### python-pip {#python-pip .unnumbered}

正常编译即可。

### marshalparser {#marshalparser .unnumbered}

正常编译即可。

### glib2 {#glib2 .unnumbered}
...
### libcroco {#libcroco .unnumbered}

找不到

## Jbig2dec {#jbig2dec .unnumbered}

正常编译即可。

### OpenJPEG2 {#openjpeg2 .unnumbered}

正常编译即可。

### graphite2 {#graphite2 .unnumbered}

新版的graphite2已经不编译docs了。

### Python-mako {#python-mako .unnumbered}

rpmbuild \--rebuild /opt/srpms/Packages/python-mako-1.2.3-9.el10.src.rpm
\--nodeps \--nocheck

### gobject-introspection {#gobject-introspection .unnumbered}

正常编译即可。

### Harfbuzz {#harfbuzz .unnumbered}
...
### T1lib {#t1lib .unnumbered}

没有找到源码包。

### T1utils {#t1utils .unnumbered}

没有找到源码包。

### Xaw3d {#xaw3d .unnumbered}

注释掉gpgverify的语句

### libXComposite {#libxcomposite .unnumbered}

正常编译即可。

### zziplib {#zziplib .unnumbered}

正常编译即可。

### teckit {#teckit .unnumbered}

正常编译即可。

### potrace {#potrace .unnumbered}

正常编译即可。

### Texlive {#texlive .unnumbered}
...
### gettext {#gettext-1 .unnumbered}

正常编译即可。

### adobe-mapping-cmap {#adobe-mapping-cmap .unnumbered}

正常编译即可。

### adobe-mapping-pdf {#adobe-mapping-pdf .unnumbered}
...
### ghostscript {#ghostscript .unnumbered}
...
### google-droid-font {#google-droid-font .unnumbered}

### libsigsegv {#libsigsegv .unnumbered}

### poppler-data {#poppler-data .unnumbered}

正常编译即可。

### poppler {#poppler .unnumbered}
